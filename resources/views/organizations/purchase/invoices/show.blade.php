									...
									<table class="table mb-0">
										<tbody>
											...
											<tr>
												...
												<td class="text-right">{{'- '.app(App\Settings\ThemeSettings::class)->currency_symbol.' '.(($invoice->discount/100) * $invoice->total)}}</td>
											</tr>
											<tr>
												<th>Tax: <span class="text-regular">({{$invoice->tax->percentage}}%)</span></th>
												<td class="text-right">{{'+ '.app(App\Settings\ThemeSettings::class)->currency_symbol.' '.(($invoice->tax->percentage/100) * ($invoice->total-(($invoice->discount/100) * $invoice->total)))}}</td>
											</tr>
											<tr>
											
											<tr>
												<th>Total:</th>
												<td class="text-right text-primary"><h5>{{app(App\Settings\ThemeSettings::class)->currency_symbol.' '.($invoice->total -(($invoice->discount/100) * $invoice->total))  + (($invoice->tax->percentage/100) * ($invoice->total-(($invoice->discount/100) * $invoice->total)))}}</h5></td>
											</tr>
										</tbody>
									</table>
								</div>
